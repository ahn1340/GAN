...
class Trainer:
    ...
    def before_train(self):
        ...
        self.wandb_logger = WandbLogger(project='GAN')

        # set number of iterations to train for discriminator and generator
        self.nG = 1  #TODO: Set this up through yaml
        self.nD = 1

        # set max iteration per epoch
        num_samples = (len(self.dataloader)-1) * self.batch_size
        one_iter_batch_size = (self.nG + self.nD) * self.batch_size
    ...
    def after_iter(self):
        """
        update AverageMeter and log results
        set description
        compute D accuracy
        """
